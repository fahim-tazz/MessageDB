...
export async function getRecentsHandler(req: Request, res: Response) {
    let offset = 0;
    if (req.query.page) {
        offset = (parseInt(req.query.page as string) - 1) * 50
    }
    ...
    ORDER BY
        m.created_at DESC
    `
    const queryResult = await postgres.query(getRecentsQuery);
...
export async function searchHandler(req: Request, res: Response) {
    ...
    const searchPage = parseInt(req.query.page as string) || 1;
    ...
    const searchQuery = `
        ...
        LIMIT 50 OFFSET ${offset};
    `;
    const queryResult = await postgres.query(searchQuery, [searchKeyword]);
    res.json(queryResult.rows);
}